...
PARAM$experimento <- "KA_C3_SEM_goss" 
...
#iteración 73 164.8 (estímulos 13746)
PARAM$finalmodel$num_iterations <- 762
PARAM$finalmodel$learning_rate <- 0.0464658535683832
PARAM$finalmodel$feature_fraction <- 0.211080709701298
PARAM$finalmodel$min_data_in_leaf <- 44463
PARAM$finalmodel$num_leaves <- 847
PARAM$finalmodel$neg_bagging_fraction <- 0.203785418463562
PARAM$finalmodel$feature_fraction_bynode <- 0.896969550248634
PARAM$finalmodel$max_depth <- 36
PARAM$finalmodel$top_rate <- 0.385643740702529
PARAM$finalmodel$other_rate <- 0.341116174114723
...
for (semilla in semillas) {
  ...
  modelo <- lgb.train(
    ...
    param = list(
      ...
      neg_bagging_fraction = PARAM$finalmodel$neg_bagging_fraction,
